...
func NewProtocol(prefix string, handler ProtocolHandler) (*Protocol, error) {
	...
	if err = handler.Initialize(nil, repo); err != nil {
		return nil, err
	}
...
loop:
	for {
		...
		case strings.HasPrefix(command, "push "):
			...
			p.push(ctx, refs[0], refs[1], false)
...
func (p *Protocol) push(ctx context.Context, src string, dst string, force bool) {
	_ = force // TODO: handle force push

	p.lazyWork = append(p.lazyWork, func() (string, error) {
		done, err := p.handler.Push(ctx, src, dst)
		if err != nil {
			return "", err
		}

		log.Debug().Str("src", src).Str("dst", dst).Msgf("push done")
...
func (p *Protocol) NewFetch() *core.Fetch {
	return core.NewFetch(p.localDir, p.tracker, p.handler.ProvideBlock)
}
